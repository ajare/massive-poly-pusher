...
| Graph execution | `RenderGraphTargets` aliases compatible non-overlapping intervals, retains compatible targets for cross-frame reuse, and supports imported backing targets. `RenderGraphExecutor` creates framebuffer views, configures MRT draw buffers, clears load operations, invalidates `DontCare` stores when supported, and invokes pass callbacks. MSAA, compute passes, simultaneous multiple mip views of one texture, and explicit read-buffer handling remain absent. Generated chains, explicit mip attachments/sampler views, and automated GPU validation are implemented. | Add multisample support, optional texture-view objects, compute/pass-type support, and read-buffer policy. |
