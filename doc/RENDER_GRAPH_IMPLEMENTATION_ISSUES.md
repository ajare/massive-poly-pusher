...
| Graph execution | `RenderGraphTargets` aliases compatible non-overlapping plan intervals and supports imported backing targets. `RenderGraphExecutor` creates framebuffer views, configures MRT draw buffers, clears load operations, and invokes pass callbacks. Cross-frame pooling, MSAA/mips, compute passes, explicit read-buffer handling, and automated GPU validation remain absent. | Add cross-frame pool retention, multisample/mip support, compute/pass-type support, read-buffer policy, and GPU tests. |
| Load/store operations | `Clear` executes through `glClearBuffer*`; `Load` and `DontCare` load safely preserve/ignore existing contents. `DontCare` store does not yet use framebuffer invalidation. | Add optional capability-guarded invalidation after GPU tests prove it is safe. |
