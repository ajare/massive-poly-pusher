...
| Graph execution | `RenderGraph` produces an ordered topology and per-version allocation/lifetime plan. `RenderGraphTargets` can allocate one `RenderTexture` per planned non-imported version, but does not bind imported targets, pool/alias allocations, compile per-pass framebuffers, or invoke callbacks. | Add imported-target binding, compiled framebuffer objects, execution context, and target pooling. |
