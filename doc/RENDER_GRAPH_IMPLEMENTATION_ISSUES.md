...
| Graph execution | `RenderGraphTargets` aliases compatible non-overlapping intervals, retains compatible targets for cross-frame reuse, and supports imported backing targets. `RenderGraphExecutor` creates framebuffer views, configures MRT draw buffers, clears load operations, invalidates `DontCare` stores when supported, and invokes pass callbacks. MSAA, explicit mip-subresource writes/views, compute passes, and explicit read-buffer handling remain absent. Generated mip chains and automated GPU validation are implemented. | Add multisample support, mip-subresource attachment views, compute/pass-type support, and read-buffer policy. |
