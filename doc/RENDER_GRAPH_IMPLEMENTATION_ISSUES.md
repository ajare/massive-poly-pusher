...
|---|---|---|
| XML graph resources | `RenderGraphParser` now parses a nested-element topology subset, but there is no `RenderGraphStream`, serializer, ResourceManager factory, shader/program/parameter parsing, depth-output parsing, imported-target mapping, or executable pass callback. XML scene passes also need a registered callback/factory name because arbitrary C++ execution callbacks cannot be serialized. | Expand the parser, add streams and serializer in `mpp-resource-parsers`, add ResourceManager factory, and add application scene-pass-factory registration. |
