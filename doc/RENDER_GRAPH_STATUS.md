...
- [~] RG2 allocation/execution: cross-frame pooling, imported bindings, capability-guarded `DontCare` invalidation, and validated same-frame transient aliasing work. The allocator aliases only physically compatible transient images whose inclusive pass intervals do not overlap, and rejects conflicting assignments. DemoSuite startup validates framebuffer output, resize, MRT locations, alias/non-alias cases, execution, and target release. MSAA, mip allocation, and compute passes remain absent.

## Not started

- [~] MSAA, mip allocation, compute passes, and broader effect/readback comparisons. Automated framebuffer completeness, colour readback, resize, MRT location readback, safe transient aliasing, execution, and target-release tests run during DemoSuite setup.
