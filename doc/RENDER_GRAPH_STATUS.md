...
- [~] Resource authoring: `RenderGraphStream` and `RenderGraphTemplate` can declare immutable programmatic graph resources through ResourceManager. The XML parser subset exists, but file-stream integration, serialization, program references, typed parameters, imported-target names, and executable bindings are not implemented.
