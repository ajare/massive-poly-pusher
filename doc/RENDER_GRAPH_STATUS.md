...
- [~] RG2 allocation/execution: cross-frame pooling, imported bindings, capability-guarded `DontCare` invalidation, and validated same-frame transient aliasing work. The allocator aliases only physically compatible transient images whose inclusive pass intervals do not overlap, and rejects conflicting assignments. DemoSuite startup validates framebuffer output, resize, MRT locations, generated mip readback, alias/non-alias cases, execution, and target release. MSAA and compute passes remain absent. Explicit colour/depth mip attachments and single-mip sampler views are implemented; binding two different views of one physical texture in the same pass is rejected until texture-view objects are supported.

## Not started

- [~] MSAA, compute passes, multiple simultaneous views of one texture, and broader effect/readback comparisons. Declared mip chains, explicit colour/depth mip attachments, temporary sampler mip views, XML round trips, dimension validation, generated-mip readback, and explicit-mip readback are tested.
