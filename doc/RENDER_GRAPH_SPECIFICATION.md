...
A graph image has shape `2D` (default) or `CUBEMAP`. A cubemap has six equally sized faces and each attachment write selects either no face (2D) or one face `0..5`; each graph image presently has one colour attachment when it is a colour image. The XML image field is `<shape>2D</shape>` or `<shape>CUBEMAP</shape>`, and an output may carry `<cubeFace>0</cubeFace>` through `<cubeFace>5</cubeFace>`. A 2D attachment must not select a face; a cubemap attachment must select one. Sampling remains the image's normal texture target.
...
| `cubeFace` | final `writeColour` / `writeDepth` argument | Required for a `CUBEMAP`, absent for `2D`; integer `0..5` mapping to `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z`. |
...
| **Physical anti-aliasing compilation** | Named outputs compile to transactional renderer-owned output plans. MSAA uses private colour/depth write targets plus automatic resolves; SSAA scales relative allocations and performs separable Lanczos downsampling. | TAA consumes resolved graph colour/depth through renderer-owned histories before SSAA downsampling; fixed-preset LDR FXAA runs last at logical output size. Sample counts stay out of graph authoring. |
...
| **Texture shapes** | Graph images support 2D and cubemap attachments; 2D arrays, 3D, layered attachment, and multisample arrays are absent. | Cubemaps provide point-light shadows; arrays/layers would support cascades, stereo/multiview, texture atlases, and many lights; 3D textures would support volumes. |
| **Attachment subresources** | Mip level and cubemap face are selectable. Array layer and attachment read-buffer selection are absent. | Array-layer selection is required for cascaded slices/arrays. Explicit read-buffer selection is useful for deterministic MRT readback/copy/resolve operations. |
