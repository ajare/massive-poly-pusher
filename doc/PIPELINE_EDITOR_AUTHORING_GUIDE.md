...
The **Process Flow** tab beside **Pipeline Hierarchy** is a read-only view of the last completely rendered pipeline frame. Its vertical solid grey spine runs from top to bottom in actual dependency-compiled pass/stage order. Same-material batch groups and optional GL State nodes are displayed as vertically connected child steps to the left of their owning pass. A faint shared background groups each pass with its child steps; renderer-owned resolve, TAA, SSAA, FXAA, and presentation work follows on the spine. Each group retains the count of exact renderer submissions it represents. Enable **GL state nodes** to insert a labelled **GL State** node whenever an actual program, texture binding, polygon, cull, blend, or depth-write transition separates submissions; it prevents grouping across the transition, and its hover text lists the recorded changes. The toggle is off by default, so material submissions remain batched across hidden state transitions. Orange labels inside each node summarize the corresponding RenderDoc GPU debug group; hover the node to see the complete exact RenderDoc label. Summaries cover render-graph passes, geometry classes, resolves, and enabled output-processing markers. Each pass node lists sampler/sample inputs left-aligned and colour/depth attachment outputs below them, right-aligned. Cubemap attachment outputs include their face number; a point-shadow update expands logically into ordered `+X`, `-X`, `+Y`, `-Y`, `+Z`, and `-Z` face passes over one imported cubemap. These nodes appear only on a dirty update; a reused domain has no face nodes. Coloured labelled edges show colour, depth, shadow, history, import, and output dependencies. AA stages carry concise in-node labels. Disabled output/AA stages remain dimmed with red borders in their correct position on the centred execution spine and show their bypass reason; disabled authored passes remain visible beside the spine. An amber warning marks authored pass positions that differ from execution positions.

Process Flow uses a fixed one-to-one scale. Use the vertical scrollbar for long flows, middle-drag to pan horizontally or scroll vertically, and **Fit All** to left-align the visible graph without resizing its nodes or text. **Refresh** requests an immediate sample; otherwise live snapshots are accepted at most every 0.25 seconds. Resource categories—authored images, imports, named outputs, MSAA resources, TAA histories, SSAA targets, and FXAA targets—are independently selectable and start hidden. Graphs above 500 visible nodes retain all work and display a non-fatal large-graph warning.

Click a pass or authored resource to open its Inspector. A batch or batch-group body selects its material; double-clicking expands its generation-safe participating scene-object list, whose entries select scene models. Material and model selections made elsewhere highlight matching batches. Pipeline or scene regeneration immediately invalidates sampled pointer identities. Pending valid edits show the last-valid flow as stale, invalid dependency state replaces the canvas with diagnostics, and telemetry/model failures never replace the last valid renderer generation.
...
The EXR must decode as a linear floating-point RGB/RGBA 2D image. Runtime generates the environment, diffuse irradiance, GGX reflection cubemaps, and split-sum BRDF LUT. The panorama uses the OpenGL convention documented in the implementation plan: longitude increases from `+X` toward `+Z`, and the panorama top maps to `+Y`. While HDR IBL is active, generated irradiance/prefilter maps take precedence and their manual fields are disabled. **BRDF LUT (advanced override)** remains available; leave it empty to use the renderer-generated LUT. Clear HDR IBL to use manual irradiance/prefilter cubemap bindings instead. The status line reports pending, cache hit, cache miss/generation, or failure; detailed errors are in Diagnostics. EXR sources are included under `hdr/` when exporting packages and resolve relative to extracted `pipeline.xml` on load.
