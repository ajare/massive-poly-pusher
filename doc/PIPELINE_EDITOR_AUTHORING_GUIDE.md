...
The **Process Flow** tab beside **Pipeline Hierarchy** is a read-only view of the last completely rendered pipeline frame. Its vertical solid grey spine runs from top to bottom in actual dependency-compiled pass/stage order. Same-material batch groups are displayed as child steps to the left of their owning pass, centred against it and linked by a straight directed connector; renderer-owned resolve, TAA, SSAA, FXAA, and presentation work follows on the spine. Each group retains the count of exact renderer submissions it represents. A labelled **GL State** node is inserted whenever an actual program, texture binding, polygon, cull, blend, or depth-write transition separates submissions; this prevents grouping across the transition, and its hover text lists the recorded changes. Orange labels inside each node summarize the corresponding RenderDoc GPU debug group; hover the node to see the complete exact RenderDoc label. Summaries cover render-graph passes, geometry classes, resolves, and enabled output-processing markers. Coloured labelled edges show colour, depth, shadow, history, import, and output dependencies. AA stages carry concise in-node labels. Disabled output/AA stages remain dimmed with red borders in their correct position on the centred execution spine and show their bypass reason; disabled authored passes remain visible beside the spine. An amber warning marks authored pass positions that differ from execution positions.
