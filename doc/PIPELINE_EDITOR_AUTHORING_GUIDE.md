...
Select an `MPP.PbrScene` pass and enable **Write emissive output (MRT)** to add the HDR `SceneEmissive` attachment. PipelineEditor routes every `MPP.BloomExtract` input to it; disable the option to restore Bloom Extract's scene-colour input. This requires two colour attachments/draw buffers and PBR material programs that write locations 0 and 1.
