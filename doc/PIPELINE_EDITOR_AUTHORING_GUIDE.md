...
The **Process Flow** tab beside **Pipeline Hierarchy** is a read-only view of the last completely rendered pipeline frame. Its vertical solid grey spine runs from top to bottom in actual dependency-compiled pass/stage order. Same-material batch groups are displayed as child steps to the right of their owning pass, centred against it and linked by a straight non-directional connector; renderer-owned resolve, TAA, SSAA, FXAA, and presentation work follows on the spine. Each group retains the count of exact renderer submissions it represents. Orange labels inside each node summarize the corresponding RenderDoc GPU debug group; hover the node to see the complete exact RenderDoc label. Summaries cover render-graph passes, geometry classes, resolves, and enabled output-processing markers. Coloured labelled edges show colour, depth, shadow, history, import, and output dependencies. Disabled output/AA stages remain dimmed in their correct position on the centred execution spine and show their bypass reason; disabled authored passes remain visible beside the spine. An amber warning marks authored pass positions that differ from execution positions.
