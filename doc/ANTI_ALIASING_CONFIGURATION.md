...
Output values accept `inherit` in addition to the corresponding global values. An explicit `off` or `false` overrides an enabled global default. Outputs in one pipeline must resolve to identical MSAA, SSAA, and TAA settings because they share rasterization, dimensions, and jitter. FXAA may vary per output.
