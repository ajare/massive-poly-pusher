	...
	try
	{
		...
		while (running)
		{
			...
			else if (openDocument && selectedBinding == -2)
			{
				...
				if (!value.hdrEquirectangular.empty()) { ImGui::SameLine(); if (ImGui::Button("Regenerate HDR IBL")) forceWorkingPreviewRebuild(); ImGui::TextDisabled("HDR IBL status: %s", !previewFailure.empty() ? "failed (see diagnostics)" : previewStale ? "pending rebuild" : "active preview; cache reuse is automatic"); }
