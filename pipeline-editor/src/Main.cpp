...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	try
	{
		...
		ResourceManager resources(&renderSystem, &logger);resources.setImageLoadFunction(mpp::app::loadImageFile); renderSystem.createCoreResources(&resources);
		...
		std::string activePipeline = "EditorUI", activeGraphResource, previewFailure;RenderTargetPtr activePreviewTarget,inspectedTarget;ImTextureID activePreviewTexture=0,inspectedTexture=0;bool inspectSelectedImage=false;int inspectedVersion=0;bool previewStale=false,documentChangedSincePreview=false; uint32_t runtimeGeneration = 0;
		auto rebuildPreview = [&]()
		{
			if (!openDocument || !openDocument->graph || openDocument->validate(renderSystem.getCaps()).hasErrors()){previewStale=!activeGraphResource.empty();previewFailure="Working document is invalid; retaining the last valid preview.";return false;}
			auto suffix = std::to_string(++runtimeGeneration), candidatePipeline="EditorPreview."+suffix, candidateGraph="PipelineEditor.Graph."+suffix;bool graphDeclared=false,pipelineDeclared=false,workspacePrepared=false;
			try
			{
				auto graphStream=std::make_shared<RenderGraphStream>(&resources);graphStream->setGraph(openDocument->graph);auto graphResource=resources.declareResource(candidateGraph,graphStream).first;graphDeclared=true;graphResource->load();graphResource->create();if(!pipelineRuntime.rebuild(clonePipeline(openDocument),viewportWidth,viewportHeight)){std::string message="Pipeline workspace rebuild failed.";for(auto const& diagnostic:pipelineRuntime.getDiagnostics().getDiagnostics())if(diagnostic.severity==DiagnosticSeverity::Error){message+=" "+diagnostic.message;break;}throw std::runtime_error(message);}workspacePrepared=true;RenderPipelineOptions previewOptions;previewOptions.mode=RenderPipelineMode::XmlGraphPbrForward;previewOptions.graphTemplate=graphResource;previewOptions.graphImports=pipelineRuntime.getImports();previewOptions.environment=pipelineRuntime.getEnvironment();auto candidatePreviewTarget=pipelineRuntime.getPresentationTarget();
				renderSystem.getOrCreateRenderPipeline(candidatePipeline,previewOptions);pipelineDeclared=true;
				if(openScene){if(!sceneRuntime.rebuild(*openScene,pipelineRuntime.getMaterialBindings(),pipelineRuntime.getInstanceOverrides())){std::string message="Preview scene rebuild failed.";for(auto const& diagnostic:sceneRuntime.getDiagnostics().getDiagnostics())if(diagnostic.severity==DiagnosticSeverity::Error){message+=" "+diagnostic.message;break;}throw std::runtime_error(message);}scene=sceneRuntime.getScene();scene->setClearColour(Colour(0.094f,0.106f,0.125f));scene->setViewport(0,0,viewportWidth,viewportHeight);setOrbitView(openScene->camera.position,openScene->camera.target);camera->setFov(openScene->camera.fov);camera->setClipDistances(openScene->camera.nearPlane,openScene->camera.farPlane);}
				auto obsoletePipeline=activePipeline,obsoleteGraph=activeGraphResource;auto obsoleteTexture=activePreviewTexture;activePipeline=candidatePipeline;activeGraphResource=candidateGraph;activePreviewTarget=candidatePreviewTarget;activePreviewTexture=0;if(auto texture=std::dynamic_pointer_cast<RenderTexture>(activePreviewTarget))activePreviewTexture=provider->registerTexture(texture);if(obsoleteTexture)provider->unregisterTexture(obsoleteTexture);if(inspectedTexture)provider->unregisterTexture(inspectedTexture);inspectedTexture=0;inspectedTarget.reset();previewStale=false;documentChangedSincePreview=false;previewFailure.clear();if(!obsoleteGraph.empty()){renderSystem.removeRenderPipeline(obsoletePipeline);resources.deleteResource(obsoleteGraph);}pipelineRuntime.accept();return true;
			}
			catch(std::exception const& error)
			{
				if(pipelineDeclared)renderSystem.removeRenderPipeline(candidatePipeline);if(workspacePrepared)pipelineRuntime.rollback();if(graphDeclared){try{resources.deleteResource(candidateGraph);}catch(...){}}previewStale=!activeGraphResource.empty();previewFailure=error.what();logger.error("Preview rebuild failed: "+previewFailure);return false;
			}
		};
		rebuildPreview();
		...
		while (running)
		{
			...
			if(openDocument&&openDocument->graph&&ImGui::TreeNodeEx("Images",ImGuiTreeNodeFlags_DefaultOpen)){for(uint32_t image=0;image<openDocument->graph->getImageCount();++image){auto info=openDocument->graph->getImageInfo({image,0});if(ImGui::Selectable(info.name.c_str(),selectedImage==(int)image)){selectedImage=(int)image;inspectedVersion=(int)openDocument->graph->getImageVersionCount(image)-1;selectedImport=-1;selectedPass=-1;selectedModel=-1;selectedLocalResource=-1;selectedExternalResource=-1;}}ImGui::TreePop();}
			...
			{
				auto before=clonePipeline(openDocument);auto& value=openDocument->localResources[(size_t)selectedLocalResource];auto& definition=value.definition;ImGui::Text("Local resource: %s",value.name.c_str());ImGui::Text("Type: %s",definition.getName().c_str());bool changed=false;auto text=[&](utils::StructuredData& data,char const* key,char const* label){char buffer[512]{};if(data.hasEntry(key))strncpy_s(buffer,data.getEntry(key).getValue().c_str(),511);if(ImGui::InputText(label,buffer,sizeof(buffer))){data.setEntryValue(key,buffer);changed=true;}};auto number=[&](utils::StructuredData& data,char const* key,char const* label,float fallback){float current=fallback;if(data.hasEntry(key))try{current=std::stof(data.getEntry(key).getValue());}catch(...){}if(ImGui::InputFloat(label,&current)){data.setEntryValue(key,std::to_string(current));changed=true;}};
				if(value.kind==PbrPipelineResourceKind::Sampler){text(definition,"minFilter","Min filter");text(definition,"magFilter","Mag filter");text(definition,"wrap","Wrap");number(definition,"lodMinLevel","Minimum LOD",-1000);number(definition,"lodMaxLevel","Maximum LOD",1000);number(definition,"lodBias","LOD bias",0);number(definition,"maxAnisotropy","Maximum anisotropy",1);}else if(value.kind==PbrPipelineResourceKind::Texture){text(definition,"filename","Image file");text(definition,"target","Target");text(definition,"internalFormat","Internal format");text(definition,"colourSpace","Colour space");text(definition,"minFilter","Min filter");text(definition,"magFilter","Mag filter");text(definition,"wrap","Wrap");number(definition,"maxAnisotropy","Maximum anisotropy",1);}else if(value.kind==PbrPipelineResourceKind::PbrMaterial){if(definition.hasEntry("Surface")){auto& surface=definition.getEntry("Surface");text(surface,"baseColourFactor","Base colour factor");number(surface,"metallicFactor","Metallic",1);number(surface,"roughnessFactor","Roughness",1);text(surface,"emissiveFactor","Emissive factor");number(surface,"normalScale","Normal scale",1);number(surface,"occlusionStrength","Occlusion strength",1);text(surface,"alphaMode","Alpha mode");number(surface,"alphaCutoff","Alpha cutoff",0.5f);text(surface,"doubleSided","Double sided");}else ImGui::TextDisabled("Add a Surface block through a template before editing factors.");if(definition.hasEntry("Program"))ImGui::TextUnformatted("Program definition present (reflection validates on Apply). ");if(auto runtimeMaterial=std::dynamic_pointer_cast<PbrMaterial>(pipelineRuntime.getResolvedResource(value.name))){ImGui::Separator();ImGui::Text("Active features: %s",runtimeMaterial->getFeatureSummary().c_str());ImGui::Text("Program: %s",runtimeMaterial->getProgram()?runtimeMaterial->getProgram()->getName().c_str():"unresolved");}}else{if(definition.hasEntry("VertexShader"))ImGui::TextUnformatted("Vertex shader definition present.");if(definition.hasEntry("FragmentShader"))ImGui::TextUnformatted("Fragment shader definition present.");ImGui::TextDisabled("Shader source remains read-only; edit file/resource references in XML.");}
				if(changed){auto after=clonePipeline(openDocument);documentChangedSincePreview=true;pipelineCommands.execute(std::make_unique<PipelineSnapshotCommand>("Edit Local Resource",&openDocument,before,after));lastEditScene=false;pipelineDirty=currentPath.empty()||pipelineCommands.dirty();}
			}
			...
			ImGui::Begin("Diagnostics"); if (openDocument) { auto diagnostics=openDocument->validate(renderSystem.getCaps());diagnostics.append(resource_parsers::validatePbrPipelineResourceDefinitions(*openDocument)); if(openScene)diagnostics.append(openScene->validate());diagnostics.append(pipelineRuntime.getDiagnostics());diagnostics.append(sceneRuntime.getDiagnostics()); ImGui::Text("%zu error(s), %zu warning(s)",diagnostics.count(DiagnosticSeverity::Error),diagnostics.count(DiagnosticSeverity::Warning)); for(auto const&d:diagnostics.getDiagnostics()) ImGui::BulletText("[%s] %s",d.code.c_str(),d.message.c_str()); } else ImGui::TextColored(ImVec4(0.3f,1,0.4f,1),"No document loaded"); ImGui::End();
			...
			ImGui::Begin("Statistics");ImGui::Text("Viewport: %u x %u",viewportWidth,viewportHeight);ImGui::Text("Frame: %.3f ms (%.1f FPS)",fps>0?1000.0f/fps:0.0f,fps);if(!activeGraphResource.empty()){auto const& stats=renderSystem.getRenderPipeline(activePipeline)->getLastGraphExecutionStats();double totalCpu=0;uint64_t totalTriangles=0;for(auto const& pass:stats){totalCpu+=pass.cpuMilliseconds;totalTriangles+=pass.trianglesSubmitted;ImGui::BulletText("%s: %.3f ms CPU, %llu triangles, %llu fullscreen",pass.name.c_str(),pass.cpuMilliseconds,(unsigned long long)pass.trianglesSubmitted,(unsigned long long)pass.fullscreenQuads);}ImGui::Separator();ImGui::Text("Graph total: %.3f ms CPU, %llu submitted triangles",totalCpu,(unsigned long long)totalTriangles);}ImGui::End();
			...
			ImGui::Checkbox("Inspect selected image",&inspectSelectedImage);if(inspectSelectedImage&&openDocument&&openDocument->graph&&selectedImage>=0){auto versions=(int)openDocument->graph->getImageVersionCount((uint32_t)selectedImage);inspectedVersion=std::clamp(inspectedVersion,0,std::max(0,versions-1));ImGui::SameLine();ImGui::SetNextItemWidth(140);ImGui::SliderInt("Version",&inspectedVersion,0,std::max(0,versions-1));}
			...
			auto viewportSize=ImGui::GetContentRegionAvail();auto requestedWidth=(uint32_t)std::max(1.0f,viewportSize.x),requestedHeight=(uint32_t)std::max(1.0f,viewportSize.y);if(activePreviewTarget&&(requestedWidth!=viewportWidth||requestedHeight!=viewportHeight)){if(activePreviewTexture)provider->unregisterTexture(activePreviewTexture);pipelineRuntime.resize(requestedWidth,requestedHeight);viewportWidth=requestedWidth;viewportHeight=requestedHeight;scene->setViewport(0,0,viewportWidth,viewportHeight);camera->setAspectRatio(float(viewportWidth)/float(viewportHeight));if(auto texture=std::dynamic_pointer_cast<RenderTexture>(activePreviewTarget))activePreviewTexture=provider->registerTexture(texture);}ImTextureID displayTexture=activePreviewTexture;if(inspectSelectedImage&&openDocument&&openDocument->graph&&selectedImage>=0&&!activeGraphResource.empty()){auto target=renderSystem.getRenderPipeline(activePipeline)->getGraphImageRenderTarget({(uint32_t)selectedImage,(uint32_t)inspectedVersion});if(target!=inspectedTarget){if(inspectedTexture)provider->unregisterTexture(inspectedTexture);inspectedTexture=0;inspectedTarget=target;if(auto texture=std::dynamic_pointer_cast<RenderTexture>(target))inspectedTexture=provider->registerTexture(texture);}if(inspectedTexture)displayTexture=inspectedTexture;}if(displayTexture)ImGui::Image(displayTexture,ImVec2((float)viewportWidth,(float)viewportHeight),ImVec2(0,1),ImVec2(1,0));else ImGui::TextUnformatted(inspectSelectedImage?"Selected image is not allocated in the current generation.":"No presentation import is available."); ImGui::End();
		...
		if(activePreviewTexture)provider->unregisterTexture(activePreviewTexture);if(inspectedTexture)provider->unregisterTexture(inspectedTexture);inspectedTarget.reset();activePreviewTarget.reset();if(!activeGraphResource.empty()){renderSystem.removeRenderPipeline(activePipeline);resources.deleteResource(activeGraphResource);}
		scene->unload();scene.reset();sceneRuntime.clear();pipelineRuntime.clear(); imGuiShutdown(&backend); renderSystem.destroyCoreResources(); window.destroy(); SDL_Quit();
		return 0;
	}
