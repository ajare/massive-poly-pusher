...
class IndexedTriangleBatchDataProvider
{
...
class IndexedTriangleBatchDataProvider<PosType, TexType, mpp::mesh::DataTypeNone>
{
	size_t mNumTriangles{ 0 };
...
public:
	...
	virtual void texcoords(uint32 index, typename PosType::builtin_type& u, typename PosType::builtin_type& v) = 0;
...
template<typename PosType, typename TexType, typename ColType = mpp::mesh::DataTypeNone>
class TestIndexedTriangleBatchDataProvider : public IndexedTriangleBatchDataProvider<PosType, TexType, ColType> {};

// Specialization (to be used) for our data provider
template<>
class TestIndexedTriangleBatchDataProvider<
	mpp::mesh::DataTypeFloat,
	mpp::mesh::DataTypeFloat,
	mpp::mesh::DataTypeUnsignedByte>
	: public IndexedTriangleBatchDataProvider<
	mpp::mesh::DataTypeFloat,
	mpp::mesh::DataTypeFloat,
	mpp::mesh::DataTypeUnsignedByte>
{
...
public:
	...
	TestIndexedTriangleBatchDataProvider(mpp::RenderSystem* renderSystem, size_t numTriangles)
	...
	void texcoords(uint32 index, float& u, float& v)
	{
	}
...
struct IndexedTriangleBatchRendererParams
{
...
template<typename PosType, typename TexType, typename ColType = mpp::mesh::DataTypeNone>
class IndexedTriangleBatchRenderer
{
	mpp::RenderSystem* mRenderSystem{ nullptr };
	...
	IndexedTriangleBatchDataProvider<PosType, TexType, ColType>* mDataProvider{ nullptr };

public:

	IndexedTriangleBatchRenderer(std::string const& name,
		IndexedTriangleBatchRendererParams const& params,
		IndexedTriangleBatchDataProvider<PosType, TexType, ColType>* dataProvider,
	...
	{
		mBatch = new mpp::IndexedTriangleBatch(
			name,
			{
	...
	virtual ~IndexedTriangleBatchRenderer()
	{
		delete mBatch;
	}
	...
	size_t update(size_t count)
	{
		...
		for (size_t pOffset = 0, tOffset = 0, cOffset = 0, i = 0; i < triangleCount; ++i)
		{
			...
			{
				TexType::builtin_type u, v;
				mDataProvider->texcoords(primitiveIndex, u, v);

				texBuffer[tOffset + 0] = u;
				texBuffer[tOffset + 1] = v;
			}
...
template<typename PosType, typename TexType>
class IndexedTriangleBatchRenderer<PosType, TexType, mpp::mesh::DataTypeNone>
{
	mpp::RenderSystem* mRenderSystem{ nullptr };
	...
	IndexedTriangleBatchDataProvider<PosType, TexType, mpp::mesh::DataTypeNone>* mDataProvider{ nullptr };

public:

	IndexedTriangleBatchRenderer(std::string const& name,
		IndexedTriangleBatchRendererParams const& params,
		IndexedTriangleBatchDataProvider<PosType, TexType, mpp::mesh::DataTypeNone>* dataProvider,
		mpp::ResourcePtr texture,
		size_t initialSize,
		mpp::RenderSystem* renderSystem,
		mpp::ResourceManager* resourceMgr)
