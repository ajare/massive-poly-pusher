		...
		struct Reader
		{
			...
			void parseEmitter(Data const& node, size_t index)
			{
				...
				if (node.hasEntry("Appearance"))
				{
					...
					appearance.modes[2]=uint32_t(enumeration(block,"billboard",appearancePath,ParticleBillboardMode::CameraFacing,{{"camerafacing",ParticleBillboardMode::CameraFacing},{"screenaligned",ParticleBillboardMode::ScreenAligned},{"cylindrical",ParticleBillboardMode::Cylindrical},{"axislocked",ParticleBillboardMode::AxisLocked},{"velocityaligned",ParticleBillboardMode::VelocityAligned}}));
					appearance.modes[3]=uint32_t(enumeration(block,"blendClass",appearancePath,ParticleBlendClass::Additive,{{"additive",ParticleBlendClass::Additive},{"alpha",ParticleBlendClass::Alpha}}));
