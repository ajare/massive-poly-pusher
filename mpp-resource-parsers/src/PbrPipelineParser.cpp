...
namespace mpp::resource_parsers
{
	...
	PbrPipelineDocument PbrPipelineParser::fromFile(string const& filepath)
	{
		...
		if (data.hasEntry("AmbientOcclusion"))
		{
			...
			if(ao.hasEntry("GTAO")){auto const& value=ao.getEntry("GTAO");rejectUnknown(value,{"radius","intensity","thickness","horizonBias","falloffStart","falloffEnd","sliceCount","stepsPerSlice","power","blurRadius"},"AmbientOcclusion/GTAO");auto& options=document.ambientOcclusion.gtao;if(value.hasEntry("radius"))options.radius=utils::StringUtils::parseFloat(value.getEntry("radius").getValue());if(value.hasEntry("intensity"))options.intensity=utils::StringUtils::parseFloat(value.getEntry("intensity").getValue());if(value.hasEntry("thickness"))options.thickness=utils::StringUtils::parseFloat(value.getEntry("thickness").getValue());if(value.hasEntry("horizonBias"))options.horizonBias=utils::StringUtils::parseFloat(value.getEntry("horizonBias").getValue());if(value.hasEntry("falloffStart"))options.falloffStart=utils::StringUtils::parseFloat(value.getEntry("falloffStart").getValue());if(value.hasEntry("falloffEnd"))options.falloffEnd=utils::StringUtils::parseFloat(value.getEntry("falloffEnd").getValue());if(value.hasEntry("sliceCount"))options.sliceCount=utils::StringUtils::parseInt(value.getEntry("sliceCount").getValue());if(value.hasEntry("stepsPerSlice"))options.stepsPerSlice=utils::StringUtils::parseInt(value.getEntry("stepsPerSlice").getValue());if(value.hasEntry("power"))options.power=utils::StringUtils::parseFloat(value.getEntry("power").getValue());if(value.hasEntry("blurRadius"))options.blurRadius=utils::StringUtils::parseInt(value.getEntry("blurRadius").getValue());}
		}
