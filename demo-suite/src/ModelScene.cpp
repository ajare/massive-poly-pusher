...
void ModelScene::setupImpl(mpp::RenderSystem* renderSystem, ProgramOptions const& options)
{
	...
	if (roundTrippedPipeline.name != pipelineDocument.name || roundTrippedPipeline.imports.size() != pipelineDocument.imports.size() || roundTrippedPipeline.previewOverrides.size() != pipelineDocument.previewOverrides.size() || roundTrippedPipeline.previewOverrides.front().values.getNumUniforms() != pipelineDocument.previewOverrides.front().values.getNumUniforms() || !roundTrippedPipeline.graph || roundTrippedPipeline.graph->getPassCount() != pipelineDocument.graph->getPassCount())
