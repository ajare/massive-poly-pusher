...
void ModelScene::setupImpl(mpp::RenderSystem* renderSystem, ProgramOptions const& options)
{
	...
	if(pipelineDocument.outputs.size()!=1||pipelineDocument.outputs.front().name!="Main"||pipelineDocument.outputs.front().image!="Presentation"||pipelineDocument.outputs.front().taaDepth!="SceneDepth")throw std::runtime_error("Native PbrPipeline named output parsing failed.");
	{auto invalid=pipelineDocument;invalid.outputs.push_back(invalid.outputs.front());if(!invalid.validate().hasErrors())throw std::runtime_error("Duplicate pipeline output was accepted.");}
	{auto invalid=pipelineDocument;auto second=invalid.outputs.front();second.name="Auxiliary";second.antiAliasing.msaa=AntiAliasingSamples::Off;invalid.outputs.push_back(second);AntiAliasingDefaults defaults;defaults.msaa=AntiAliasingSamples::X4;if(!invalid.validateOutputAntiAliasing(defaults).hasErrors())throw std::runtime_error("Inherited/explicit output anti-aliasing mismatch was accepted.");}
	{auto valid=pipelineDocument;valid.outputs.front().antiAliasing.fxaa=true;auto second=valid.outputs.front();second.name="Auxiliary";second.antiAliasing.fxaa=false;valid.outputs.push_back(second);if(valid.validateOutputAntiAliasing({}).hasErrors())throw std::runtime_error("Per-output FXAA variation was rejected.");}
	{auto invalid=pipelineDocument;invalid.outputs.front().image="SceneHdr";invalid.outputs.front().taaDepth.clear();invalid.outputs.front().antiAliasing.taa=true;invalid.outputs.front().antiAliasing.fxaa=true;if(!invalid.validate().hasErrors())throw std::runtime_error("Invalid TAA/FXAA output combination was accepted.");}
	{auto invalid=pipelineDocument;invalid.outputs.front().image="SceneHdr";invalid.outputs.front().antiAliasing.taa=false;invalid.outputs.front().antiAliasing.fxaa=true;if(!invalid.validateOutputAntiAliasing({}).hasErrors())throw std::runtime_error("HDR FXAA output was accepted.");}
	auto localizedPipeline=pipelineDocument;mpp::PbrPipelineResourceDocument nestedReference;nestedReference.name="Nested.Reference";nestedReference.kind=mpp::PbrPipelineResourceKind::Sampler;nestedReference.definition=mpp::data::StructuredData("Sampler");nestedReference.definition.addEntry("name",nestedReference.name);mpp::data::StructuredData nested("Nested");nested.addEntry("resource","DemoPreview::LinearClamp");nestedReference.definition.addEntry("Nested",nested);localizedPipeline.localResources.push_back(nestedReference);if(!localizedPipeline.makeLocalCopy("DemoPreview::LinearClamp","Preview.CopiedLinearClamp")||localizedPipeline.localResources.size()!=pipelineDocument.localResources.size()+2||localizedPipeline.localResources.back().definition.getEntry("name").getValue()!="Preview.CopiedLinearClamp"||localizedPipeline.localResources.at(localizedPipeline.localResources.size()-2).definition.getEntry("Nested").getEntry("resource").getValue()!="Preview.CopiedLinearClamp")throw std::runtime_error("PbrPipeline Make Local Copy failed.");
	auto pipelineRoundTrip = std::filesystem::temp_directory_path() / "mpp-pipeline-roundtrip.xml";
	auto serializedPipeline=pipelineDocument;serializedPipeline.outputs.front().antiAliasing.msaa=AntiAliasingSamples::X2;serializedPipeline.outputs.front().antiAliasing.ssaa=AntiAliasingSamples::X8;serializedPipeline.outputs.front().antiAliasing.taa=true;serializedPipeline.outputs.front().antiAliasing.fxaa=false;
	mpp::resource_parsers::PbrPipelineSerializer::toFile(serializedPipeline, pipelineRoundTrip.string());
	...
	{std::ofstream legacy(pipelineRoundTrip,std::ios::trunc);legacy<<"<RenderGraph><Images><Image><name>Legacy</name><format>RGBA8</format><usage>colourAttachment</usage><samples>4</samples></Image></Images><Passes/></RenderGraph>";}bool rejectedLegacySamples=false;try{mpp::resource_parsers::PbrPipelineDocumentLoader::fromFile(pipelineRoundTrip.string());}catch(std::exception const& error){rejectedLegacySamples=std::string(error.what()).find("named output <AntiAliasing>")!=std::string::npos;}if(!rejectedLegacySamples)throw std::runtime_error("Legacy RenderGraph <samples> was not rejected with a migration diagnostic.");
	std::filesystem::remove(pipelineRoundTrip);
	{auto invalidMsaaMip=pipelineDocument;invalidMsaaMip.graph=std::make_shared<mpp::RenderGraph>(*pipelineDocument.graph);for(uint32_t image=0;image<invalidMsaaMip.graph->getImageCount();++image){auto info=invalidMsaaMip.graph->getImageInfo({image,0});if(!info.desc.external&&mpp::hasGraphImageUsage(info.desc.usage,mpp::GraphImageUsage::ColourAttachment)){info.desc.mipLevels=2;invalidMsaaMip.graph->setImageDesc({image,0},info.desc);break;}}invalidMsaaMip.outputs.front().antiAliasing.msaa=mpp::AntiAliasingSamples::X2;if(!invalidMsaaMip.validateOutputAntiAliasing({},nullptr).hasErrors())throw std::runtime_error("MSAA attachment mip-chain validation failed.");}
	if (roundTrippedPipeline.name != pipelineDocument.name || roundTrippedPipeline.imports.size() != pipelineDocument.imports.size() || roundTrippedPipeline.outputs.size() != pipelineDocument.outputs.size() || roundTrippedPipeline.outputs.empty() || roundTrippedPipeline.outputs.front().name != pipelineDocument.outputs.front().name || roundTrippedPipeline.outputs.front().image != pipelineDocument.outputs.front().image || roundTrippedPipeline.outputs.front().taaDepth != pipelineDocument.outputs.front().taaDepth || roundTrippedPipeline.outputs.front().antiAliasing.msaa != serializedPipeline.outputs.front().antiAliasing.msaa || roundTrippedPipeline.outputs.front().antiAliasing.ssaa != serializedPipeline.outputs.front().antiAliasing.ssaa || roundTrippedPipeline.outputs.front().antiAliasing.taa != serializedPipeline.outputs.front().antiAliasing.taa || roundTrippedPipeline.outputs.front().antiAliasing.fxaa != serializedPipeline.outputs.front().antiAliasing.fxaa || roundTrippedPipeline.localResources.size() != pipelineDocument.localResources.size() || roundTrippedPipeline.localResources.back().definition.getEntry("wrap").getValue() != pipelineDocument.localResources.back().definition.getEntry("wrap").getValue() || roundTrippedPipeline.extensions.size() != pipelineDocument.extensions.size() || roundTrippedPipeline.extensions.front().payload.getEntry("EditorMetadata").getEntry("category").getValue() != pipelineDocument.extensions.front().payload.getEntry("EditorMetadata").getEntry("category").getValue() || roundTrippedPipeline.previewOverrides.size() != pipelineDocument.previewOverrides.size() || roundTrippedPipeline.previewOverrides.front().values.getNumUniforms() != pipelineDocument.previewOverrides.front().values.getNumUniforms() || roundTrippedPipeline.bloom.enabled != pipelineDocument.bloom.enabled || roundTrippedPipeline.bloom.blurPasses != pipelineDocument.bloom.blurPasses || !roundTrippedPipeline.graph || roundTrippedPipeline.graph->getPassCount() != pipelineDocument.graph->getPassCount())
		throw std::runtime_error("Native PbrPipeline XML round trip failed.");
	{mpp::resource_parsers::PbrPipelineRuntime runtime(renderSystem,resourceMgr);auto runtimeDocument=std::make_shared<mpp::PbrPipelineDocument>(pipelineDocument);if(!runtime.rebuild(runtimeDocument,640,360)||!runtime.getEnvironment()||!runtime.getPresentationTarget()||runtime.getImports().find("shadowDepth")==runtime.getImports().end()||runtime.getMaterialBindings().size()!=3||runtime.getInstanceOverrides().find("Sphere.0.0")==runtime.getInstanceOverrides().end())throw std::runtime_error("Transactional PbrPipeline runtime resolution failed.");runtime.accept();auto retained=runtime.getDocument();auto invalid=std::make_shared<mpp::PbrPipelineDocument>(pipelineDocument);mpp::PbrPipelineResourceDocument bad;bad.name="Invalid.Sampler";bad.kind=mpp::PbrPipelineResourceKind::Sampler;bad.definition=mpp::data::StructuredData("Sampler");bad.definition.addEntry("name",bad.name);bad.definition.addEntry("minFilter","INVALID_FILTER");invalid->localResources.push_back(bad);if(runtime.rebuild(invalid,640,360)||runtime.getDocument()!=retained)throw std::runtime_error("PbrPipeline runtime did not retain its valid generation after candidate failure.");runtime.clear();}
	auto pipelineResource = resourceMgr->declareResource("PBR.EditorPipelineTest", std::make_shared<mpp::resource_parsers::FilePbrPipelineStream>(resourceMgr, options.resourceLocation + "shared/pbr/templates/Full.pipeline.xml")).first;
	...
	if (roundTrippedScene.layers != sceneDocument.layers || roundTrippedScene.models.size() != sceneDocument.models.size() || roundTrippedScene.lights.size() != sceneDocument.lights.size() || roundTrippedScene.models.front().source != sceneDocument.models.front().source || roundTrippedScene.models.front().primitive.radius != sceneDocument.models.front().primitive.radius || roundTrippedScene.models.front().primitive.resolution != sceneDocument.models.front().primitive.resolution || roundTrippedScene.lights.back().type != sceneDocument.lights.back().type || roundTrippedScene.lights.front().castsShadows != sceneDocument.lights.front().castsShadows) throw std::runtime_error("Native Scene XML round trip failed.");
	auto sceneTemplateResource = resourceMgr->declareResource("PBR.EditorSceneTest", std::make_shared<mpp::resource_parsers::FileSceneStream>(resourceMgr, (std::filesystem::path(pipelineDocument.sourcePath).parent_path() / pipelineDocument.previewScene).string())).first;
	...
	renderSystem->infoMessage("Render graph GPU format/raster/state/stats/framebuffer/resize/MRT/MSAA/TAA/SSAA-Lanczos/FXAA/mip/alias/lifetime tests passed.");
}
