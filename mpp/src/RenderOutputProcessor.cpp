...
namespace mpp
{
	...
	void RenderOutputProcessor::present(string const& outputName,RenderTargetPtr const& destination,RenderTargetPtr const& source,RenderTargetPtr const& depthSource,TaaFrameContext const* taaFrame)
	{
		...
		size_t workIndex=0;if(state.plan.antiAliasing.ssaa!=AntiAliasingSamples::Off){if(state.work.size()<2)THROW_MPP("SSAA output is missing Lanczos work targets.",__LINE__,__FILE__,__func__);mRenderSystem->renderSsaaLanczos(texture,state.work[workIndex],glm::vec2(1,0));texture=dynamic_cast<RenderTexture*>(state.work[workIndex++].get());mRenderSystem->renderSsaaLanczos(texture,state.work[workIndex],glm::vec2(0,1));texture=dynamic_cast<RenderTexture*>(state.work[workIndex++].get());}if(state.plan.antiAliasing.fxaa){if(workIndex>=state.work.size())THROW_MPP("FXAA output is missing its LDR work target.",__LINE__,__FILE__,__func__);mRenderSystem->renderFxaa(texture,state.work[workIndex]);texture=dynamic_cast<RenderTexture*>(state.work[workIndex].get());}mRenderSystem->setProjection2dOrthographic();mRenderSystem->resetTransform();mRenderSystem->scaleTransform2d(glm::vec2((float)destination->getWidth()/mRenderSystem->getWindowWidth(),(float)destination->getHeight()/mRenderSystem->getWindowHeight()));mRenderSystem->setRenderTarget(destination);mRenderSystem->setViewport(0,0,destination->getWidth(),destination->getHeight());mRenderSystem->renderFullscreenQuad(texture,BlendMode::One,BlendMode::Zero);
	}
