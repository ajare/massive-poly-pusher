...
namespace mpp
{
	...
	void RenderOutputProcessor::present(string const& outputName,RenderTargetPtr const& destination,RenderTargetPtr const& source,RenderTargetPtr const& depthSource,TaaFrameContext const* taaFrame)
	{
		...
		size_t workIndex=0;if(state.plan.antiAliasing.ssaa!=AntiAliasingSamples::Off){if(state.work.size()<2)THROW_MPP("SSAA output is missing Lanczos work targets.",__LINE__,__FILE__,__func__);if(flow)try{mRenderSystem->recordRenderFlowEvent(RenderFlowEventKind::SsaaHorizontal,outputName+" / SSAA Horizontal",{},true,{},outputName,false,{resource(textureName,currentTarget,colourFormat)},{resource(outputName+".Work0",state.work[workIndex],colourFormat)});}catch(...){mRenderSystem->failRenderFlowCapture();}mRenderSystem->renderSsaaLanczos(texture,state.work[workIndex],glm::vec2(1,0));texture=dynamic_cast<RenderTexture*>(state.work[workIndex].get());currentTarget=state.work[workIndex++];if(flow)try{textureName=outputName+".Work0";}catch(...){mRenderSystem->failRenderFlowCapture();}if(flow)try{mRenderSystem->recordRenderFlowEvent(RenderFlowEventKind::SsaaVertical,outputName+" / SSAA Vertical",{},true,{},outputName,false,{resource(textureName,currentTarget,colourFormat)},{resource(outputName+".Work1",state.work[workIndex],colourFormat)});}catch(...){mRenderSystem->failRenderFlowCapture();}mRenderSystem->renderSsaaLanczos(texture,state.work[workIndex],glm::vec2(0,1));texture=dynamic_cast<RenderTexture*>(state.work[workIndex].get());currentTarget=state.work[workIndex++];if(flow)try{textureName=outputName+".Work1";}catch(...){mRenderSystem->failRenderFlowCapture();}}else disabled(RenderFlowEventKind::SsaaHorizontal,"SSAA");if(state.plan.antiAliasing.fxaa){if(workIndex>=state.work.size())THROW_MPP("FXAA output is missing its LDR work target.",__LINE__,__FILE__,__func__);if(flow)try{mRenderSystem->recordRenderFlowEvent(RenderFlowEventKind::Fxaa,outputName+" / FXAA",{},true,{},outputName,false,{resource(textureName,currentTarget,colourFormat)},{resource(outputName+".Work"+to_string(workIndex),state.work[workIndex],colourFormat)});}catch(...){mRenderSystem->failRenderFlowCapture();}mRenderSystem->renderFxaa(texture,state.work[workIndex]);texture=dynamic_cast<RenderTexture*>(state.work[workIndex].get());currentTarget=state.work[workIndex];if(flow)try{textureName=outputName+".Work"+to_string(workIndex);}catch(...){mRenderSystem->failRenderFlowCapture();}}else disabled(RenderFlowEventKind::Fxaa,"FXAA");if(flow)try{mRenderSystem->recordRenderFlowEvent(RenderFlowEventKind::Presentation,outputName+" / Presentation",{},true,{},outputName,false,{resource(textureName,currentTarget,colourFormat)},{resource(outputName+".Destination",destination,colourFormat)});}catch(...){mRenderSystem->failRenderFlowCapture();}mRenderSystem->setProjection2dOrthographic();mRenderSystem->resetTransform();mRenderSystem->scaleTransform2d(glm::vec2((float)destination->getWidth()/mRenderSystem->getWindowWidth(),(float)destination->getHeight()/mRenderSystem->getWindowHeight()));mRenderSystem->setRenderTarget(destination);mRenderSystem->setViewport(0,0,destination->getWidth(),destination->getHeight());mRenderSystem->renderFullscreenQuad(texture,BlendMode::One,BlendMode::Zero);
	}
