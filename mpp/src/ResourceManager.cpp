...
namespace mpp
{
	...
	*/
	...
	ResourcePtr ResourceManager::declareAndAcquireResource(string const& name, ResourceStreamPtr resourceStream, bool loadStream, uint32_t quality)
	{
		auto res = declareResource(name, resourceStream, loadStream, quality);
		res->acquire();
		return res;
	}
