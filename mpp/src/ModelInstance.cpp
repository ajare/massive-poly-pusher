...
namespace mpp
{
	 ...
	 *
	 */
	ModelInstance::ModelInstance(Model const& model, glm::mat4 const& modelCameraProjMatrix, glm::mat3 const& normalMatrix, glm::vec2 const& halfWindowSize)
	{
		...
		for (int i = 0; i < model.getNumMeshes(); ++i)
		{
			...
			MeshInstance* mi = new MeshInstance(mesh, modelCameraProjMatrix, normalMatrix, halfWindowSize);
	...
	*
	*/
	ModelInstance::ModelInstance(Model const& model, glm::mat4 const& modelCameraProjMatrix, glm::mat3 const& normalMatrix)
	{
		// Iterate over model and create MeshInstances
		for (int i = 0; i < model.getNumMeshes(); ++i)
		{
			...
			MeshInstance* mi = new MeshInstance(mesh, modelCameraProjMatrix, normalMatrix);
	 ...
	 *
	 */
	ModelInstance::ModelInstance(Model const& model, glm::mat4 const& modelCameraProjMatrix, glm::vec2 const& halfWindowSize)
	{
		// Iterate over model and create MeshInstances
		for (int i = 0; i < model.getNumMeshes(); ++i)
		{
			...
			MeshInstance* mi = new MeshInstance(mesh, modelCameraProjMatrix, halfWindowSize);
			mMeshInstances[mesh->getName()] = mi;
			mOrderedMeshInstances.push_back(mi);
		}
	}
