...
namespace mpp
{
	...
	bool runRenderGraphGpuTests(RenderSystem* renderSystem, std::string* failure)
	{
		...
		try
		{
			...
			if(std::string(renderFlowEventKindName(RenderFlowEventKind::BatchSubmission))!="batch submission"||std::string(renderFlowEventKindName(RenderFlowEventKind::Presentation))!="presentation")return fail("render-flow event naming contract failed");
