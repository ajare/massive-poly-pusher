...
namespace mpp
{
	...
	{
		if(scene){scene->unload();scene.reset();}for(auto const& name:names)if(mResourceManager->getResource(name,true)){try{mResourceManager->deleteResourceTree(name);}catch(...){}}names.clear();
	}
	void SceneRuntime::clear(){clearResources(mScene,mResourceNames);mDiagnostics.clear();mModelTriangles.clear();mUniqueTriangles=0;}

	bool SceneRuntime::rebuild(SceneDocument const& document,std::map<std::string,ResourcePtr> const& materialBindings,std::map<std::string,UniformCollection> const& instanceOverrides)
	{
