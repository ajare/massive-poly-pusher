...
namespace mpp
{
	...
	{
		mDiagnostics=document.validate();if(mDiagnostics.hasErrors())return false;ScenePtr candidate=std::make_shared<Scene>(mRenderSystem);std::vector<std::string> candidateNames;auto generation=std::to_string(++mGeneration),prefix="SceneRuntime."+generation+".";
		try
		{
			...
			for(auto const& authored:document.models)
			{
				...
				ResourcePtr model;auto modelName=prefix+authored.id;if(authored.source==SceneModelSource::MppModel){auto path=std::filesystem::path(authored.file);if(!path.is_absolute())path=std::filesystem::path(document.sourcePath).parent_path()/path;if(std::filesystem::exists(path)){try{auto stream=std::make_shared<MppModelStream>(mResourceManager,path.string());model=mResourceManager->declareResource(modelName+".Source",stream).first;candidateNames.push_back(modelName+".Source");model->load();}catch(std::exception const& error){mDiagnostics.warning("MPP-SCENE-RUNTIME-002","Model load failed; using placeholder: "+std::string(error.what()),{document.sourcePath},authored.id);model.reset();}}if(!model){mDiagnostics.warning("MPP-SCENE-RUNTIME-003","Missing model uses a placeholder box.",{document.sourcePath},authored.id);model=makePrimitive(modelName+".Placeholder",SceneModelSource::Box);}}else model=makePrimitive(modelName,authored.source);
				auto instance=candidate->add3dModel(model);instance->resetTransform();instance->translate(authored.translation);instance->rotateSelf(glm::radians(authored.rotationDegrees.x),glm::vec3(1,0,0));instance->rotateSelf(glm::radians(authored.rotationDegrees.y),glm::vec3(0,1,0));instance->rotateSelf(glm::radians(authored.rotationDegrees.z),glm::vec3(0,0,1));instance->scale(authored.scale);uint32_t flags=0;if(authored.visible)flags|=ModelRenderParams::Flag_Visible;if(authored.shadowCaster)flags|=ModelRenderParams::Flag_CastShadows;instance->getParams()->setModelFlags(flags);if(authored.source!=SceneModelSource::MppModel)instance->getParams()->setModelMaterial(material);
			}
		}
		...
		auto previous=mScene;auto previousNames=std::move(mResourceNames);mScene=std::move(candidate);mResourceNames=std::move(candidateNames);clearResources(previous,previousNames);return true;
	}
