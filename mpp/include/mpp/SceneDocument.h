...
class _MPPAPI SceneDocument { public: static constexpr uint32_t CurrentVersion=1; uint32_t version{1}; std::string sourcePath; std::string name; std::string environmentBinding; SceneCameraDocument camera; std::vector<std::string> layers; std::vector<SceneModelDocument> models; std::vector<SceneLightDocument> lights; uint64_t getKnownTriangleCount() const; size_t getUnknownTriangleModelCount() const; std::optional<glm::vec3> getShadowLightDirection() const; DiagnosticBag validate() const; };
}
